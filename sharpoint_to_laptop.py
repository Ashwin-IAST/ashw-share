...
if "access_token" not in result:
    print("❌ Authentication failed. Ensure interactive authentication is possible or consider a different MSAL flow for CI/CD.")
    sys.exit(1)
...
if site_info_response.status_code != 200:
    print(f"❌ Failed to retrieve SharePoint site info. Status: {site_info_response.status_code}, Response: {site_info_response.text}")
    sys.exit(1)
...
if drive_response.status_code != 200:
    print(f"❌ Failed to get drive info. Status: {drive_response.status_code}, Response: {drive_response.text}")
    sys.exit(1)
